...
APP=Path(os.environ.get('CHINESE_STUDY_DIR','/opt/apps/chinese-study')).resolve(); HOST=os.environ.get('CHINESE_STUDY_HOST','127.0.0.1'); PORT=int(os.environ.get('CHINESE_STUDY_PORT','8910'))
KEY=os.environ.get('MINIMAX_API_KEY','').strip(); API_HOST=os.environ.get('MINIMAX_API_HOST','https://api.minimax.io').rstrip('/'); MODEL=(os.environ.get('MINIMAX_MODEL','MiniMax-M3').strip() or 'MiniMax-M3')
TEXT_URL=API_HOST+'/anthropic/v1/messages'; VLM_URL=API_HOST+'/v1/coding_plan/vlm'; MAX_REQUEST=18*1024*1024; MAX_FILE=12*1024*1024
...
buildSession=function(){const id=state?.strictTopicNext;if(!id)return oldBuild();state.strictTopicNext=null;remember();const t=topic(id);if(!t)return oldBuild();const vw=words(t).sort((a,b)=>prio(b)-prio(a)).slice(0,5),li=shuffled(vw).slice(0,2),gr=shuffled(t.grammar||[]).slice(0,3),rd=shuffled(t.readings||[]).slice(0,2),bu=shuffled(t.builds||[]).slice(0,3),pr=shuffled(t.productions||[]).slice(0,2);lessonSteps=[vw[0]&&{type:'vocab',w:vw[0]},gr[0]&&{type:'grammar',x:gr[0]},li[0]&&{type:'listening',w:li[0]},vw[1]&&{type:'vocab',w:vw[1]},rd[0]&&{type:'reading',x:rd[0]},bu[0]&&{type:'sentence',x:bu[0]},vw[2]&&{type:'vocab',w:vw[2]},gr[1]&&{type:'grammar',x:gr[1]},li[1]&&{type:'listening',w:li[1]},vw[3]&&{type:'vocab',w:vw[3]},pr[0]&&{type:'production',x:pr[0]},bu[1]&&{type:'sentence',x:bu[1]},vw[4]&&{type:'vocab',w:vw[4]},rd[1]&&{type:'reading',x:rd[1]},gr[2]&&{type:'grammar',x:gr[2]},pr[1]&&{type:'production',x:pr[1]},bu[2]&&{type:'sentence',x:bu[2]}].filter(Boolean);lessonPos=0;lessonStats={vocab:0,listening:0,grammar:0,reading:0,sentence:0,production:0}};
...
window.studyLegacyMaterial=function(mid){const m=(state.materials||[]).find(x=>String(x.id)===String(mid));if(!m)return;const ids=(m.wordIds||[]).filter(id=>WORDS.some(w=>String(w.id)===String(id)));if(!ids.length){return reanalyzeLegacyMaterial(mid)}const id='legacy_'+String(m.id).replace(/[^a-zA-Z0-9_-]/g,'_');let t=topic(id);if(!t){t={id,title:m.title||'Сохранённый текст',pinyin:'',ru:'',wordIds:ids,grammar:[],readings:[],builds:[],productions:[],sourceText:m.preview||'',sourcePinyin:''};if(!Array.isArray(state.customTopics))state.customTopics=[];state.customTopics.push(t);remember()}studyMaterialOnly(id)};
window.reanalyzeLegacyMaterial=function(mid){const m=(state.materials||[]).find(x=>String(x.id)===String(mid));const btn=document.getElementById('showMaterialAdd');if(btn)btn.click();setTimeout(()=>{const ta=document.getElementById('aimText')||document.getElementById('materialText');if(ta&&m?.preview)ta.value=m.preview},50)};
function enhance(){const holder=$('#customMaterials');if(!holder)return;const items=[...(state.materials||[])].reverse();const h=$('.sectionhead h2',holder);if(h)h.textContent='Сохранённые материалы';let help=$('.saved-material-help',holder);if(!help){help=document.createElement('div');help.className='topic-study-note saved-material-help';help.innerHTML='<b>Как изучать:</b> нажми «Изучать этот материал» — откроется отдельный урок только по нему. «Показать текст» — просто посмотреть исходник. Для старых записей без полноценной темы можно учить сохранённые слова или переразобрать текст через MiniMax.';const head=$('.sectionhead',holder);if(head)head.after(help);else holder.prepend(help)}
$$('#customMaterials .topiccard').forEach((card,i)=>{const m=items[i];if(!m)return;const id=m.topicId;let actions=$('.custom-topic-actions',card);if(!actions){actions=document.createElement('div');actions.className='custom-topic-actions';card.appendChild(actions)}const existing=$$('.study-only-btn,.legacy-study-btn,.reanalyze-btn',actions);existing.forEach(x=>x.remove());if(id&&topic(id)){const b=document.createElement('button');b.className='primary study-only-btn';b.textContent='Изучать этот материал';b.onclick=()=>studyMaterialOnly(id);actions.prepend(b);const mixed=$$('button',actions).find(x=>x!==b&&/Начать тему|Изучать эту тему|В фокус|Добавить в общие/.test(x.textContent||''));if(mixed){mixed.classList.remove('primary');mixed.classList.add('ghost');mixed.textContent='Добавить в общие уроки'}}else if((m.wordIds||[]).length){const b=document.createElement('button');b.className='primary legacy-study-btn';b.textContent='Изучать слова из текста';b.onclick=()=>studyLegacyMaterial(m.id);actions.prepend(b)}else{const b=document.createElement('button');b.className='primary reanalyze-btn';b.textContent='Переразобрать через MiniMax';b.onclick=()=>reanalyzeLegacyMaterial(m.id);actions.prepend(b)}if(!$('.separate-note',card)){const n=document.createElement('div');n.className='topic-study-note separate-note';n.textContent=id&&topic(id)?'Отдельный урок использует только слова и задания этого материала.':'Это старая запись: если полной темы нет, сайт использует сохранившиеся слова или предлагает переразбор.';actions.after(n)}});
const shop=$('#topic-shopping');if(shop&&!$('.study-shop-only',shop)){const ref=$('#startShoppingTopic',shop),b=document.createElement('button');b.className='primary study-shop-only';b.textContent='Изучать отдельно';b.onclick=()=>studyMaterialOnly('shopping');if(ref){ref.classList.remove('primary');ref.classList.add('ghost');ref.textContent='Добавить в общие уроки';ref.before(b)}else shop.appendChild(b)}}
const oldRender=renderMaterials;renderMaterials=function(){oldRender();enhance()};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(enhance,0));else setTimeout(enhance,0);
})();'''
...
def vlm_read_image(mime,raw,page_label=''):
 ...
 resp=http_json(VLM_URL,payload,{'Authorization':'Bearer '+KEY,'MM-API-Source':'Minimax-MCP','Content-Type':'application/json'},120);text=str(resp.get('content') or '').strip()
...
def pdf_to_images(raw):
 ...
 with tempfile.TemporaryDirectory(prefix='chinese-pdf-') as td:
  src=Path(td)/'input.pdf';src.write_bytes(raw);prefix=str(Path(td)/'page');proc=subprocess.run(['pdftoppm','-jpeg','-f','1','-l','8','-r','130','-scale-to','1800',str(src),prefix],capture_output=True,timeout=60)
...
def text_analyze(extracted,note=''):
 ...
 payload={'model':MODEL,'max_tokens':14000,'system':SYSTEM_PROMPT,'messages':[{'role':'user','content':user_text}]};resp=http_json(TEXT_URL,payload,{'X-Api-Key':KEY,'Authorization':'Bearer '+KEY,'Content-Type':'application/json','anthropic-version':'2023-06-01'},220)
 ...
 out=parse_model_json('\n'.join(parts));req=['title_cn','title_pinyin','title_ru','summary_ru','source_text_cn','source_pinyin','words','grammar','readings','builds','productions'];missing=[k for k in req if k not in out]
...
class Handler(SimpleHTTPRequestHandler):
 server_version='ChineseStudy/4.2'
 ...
 def do_GET(self):
  ...
  if path.rstrip('/')=='/api/health':return self.send_json(200,{'ok':True,'version':'4.2','provider':'MiniMax','ai_configured':bool(KEY),'model':MODEL,'vision':'coding_plan/vlm','separate_topic_mode':True,'saved_material_study':True})
  if path=='/topic-study.js':return self.send_bytes(200,TOPIC_STUDY_JS.encode('utf-8'),'application/javascript; charset=utf-8')
  if path in('/','/index.html'):
   p=APP/'index.html'
   if p.is_file():
    html=p.read_text(encoding='utf-8');html=re.sub(r'<script src="topic-study\.js\?v=[^"]+"></script>\s*','',html);html=html.replace('</body>','<script src="topic-study.js?v=4.2"></script>\n</body>');return self.send_bytes(200,html.encode('utf-8'),'text/html; charset=utf-8','no-cache')
  return super().do_GET()
 def do_POST(self):
...
if __name__=='__main__':
 APP.mkdir(parents=True,exist_ok=True);print(f'Chinese Study 4.2 + MiniMax on http://{HOST}:{PORT}',flush=True);ThreadingHTTPServer((HOST,PORT),Handler).serve_forever()
